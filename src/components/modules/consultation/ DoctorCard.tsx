      ...
      <Card className="overflow-hidden hover:shadow-lg transition-shadow">
        <CardHeader className="pb-3">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <CardTitle className="text-lg line-clamp-1">Dr. {doctor.name}</CardTitle>
              <CardDescription className="line-clamp-1">{doctor.designation}</CardDescription>

              <div className="flex items-center gap-2 mt-2">
                <div className="flex items-center gap-1">
                  <Star className="h-4 w-4 fill-yellow-400 text-yellow-400" />
                  <span className="text-sm font-medium">{doctor.averageRating?.toFixed(1) || "N/A"}</span>
                </div>
        ...
        <CardContent className="space-y-3 pb-3">
          ...
          <div className="text-sm">
            ...
            <p className="text-muted-foreground line-clamp-2">{doctor.qualification}</p>
          </div>

          {doctor.doctorSpecialties && doctor.doctorSpecialties.length > 1 && (
            <div className="flex flex-wrap gap-1">
              {doctor.doctorSpecialties.slice(1, 3).map((specialty) => (
                <Badge key={specialty.specialtiesId} variant="outline" className="text-xs">
