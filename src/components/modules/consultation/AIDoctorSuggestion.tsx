...
import {Award, Briefcase, DollarSign, Loader2, MapPin, Sparkles, Star, Stethoscope, User} from "lucide-react";
  ...
  const handleGetSuggestion = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const doctors = Array.isArray(response.data) ? response.data : [response.data];
        setSuggestedDoctors(doctors);
        setShowSuggestions(true);
        toast.success("AI suggestions generated successfully!");
      } else toast.error(response.message || "Failed to get AI suggestions");
    } catch (error) {
      ...
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <CardDescription className="text-primary/80">Describe your symptoms and get AI-powered doctor recommendations</CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-4">
        <div>
          ...
          <div className="flex justify-between items-center mt-1">
            ...
            <p className="text-xs text-primary font-medium">Minimum 5 characters required</p>
          </div>
        </div>

        <Button onClick={handleGetSuggestion} disabled={isLoading || symptoms.trim().length < 5} className="w-full shadow-md" size="lg">
          {isLoading ? (
            <>
          ...
          <div className="space-y-4 p-4 bg-white rounded-lg border-2 border-primary/20 shadow-sm">
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 space-y-2 min-h-[180px] flex flex-col">
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="font-semibold text-gray-900">{doctor.name || "N/A"}</h4>
                        ...
                        {doctor.designation && <p className="text-sm text-gray-600">{doctor.designation}</p>}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1 min-h-7">
                        {doctor.specialties && doctor.specialties.length > 0 ? (
                          doctor.specialties.map((specialty, idx) => (
                            <Badge key={idx} variant={idx % 2 === 0 ? "default" : "outline"}>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-primary/20 mt-auto">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-semibold text-green-700">৳{doctor.appointmentFee}</span>
            ...
            <div className="pt-2 border-t border-primary/20">
              <p className="text-xs text-center text-muted-foreground">
                ⚠️ AI suggestions are for guidance only. Please consult a medical professional for accurate diagnosis.
              </p>
            </div>
          </div>
        )}
        ...
        {showSuggestions && suggestedDoctors.length === 0 && (
          <div className="p-6 bg-white rounded-lg border-2 border-amber-200 text-center">
            <p className="text-amber-700 font-medium">No doctor recommendations found</p>
            <p className="text-sm text-muted-foreground mt-1">Try describing your symptoms differently or browse all doctors below.</p>
          </div>
        )}
      </CardContent>
    </Card>
