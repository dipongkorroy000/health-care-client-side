...
const AdminViewDetailDialog = ({open, onClose, admin}: IAdminViewDialogProps) => {
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-6">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 p-6 bg-linear-to-br from-blue-50 to-indigo-50 dark:from-blue-950 dark:to-indigo-950 rounded-lg mb-6">
            <Avatar className="h-24 w-24 border-4 border-white shadow-lg">
              ...
              <AvatarFallback className="text-2xl">{getInitials(admin?.name || "")}</AvatarFallback>
            </Avatar>
            <div className="flex-1 text-center sm:text-left">
              ...
              <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                <Badge variant={admin?.isDeleted ? "destructive" : "default"} className="text-sm">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-muted/50 p-4 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <InfoRow label="Contact Number" value={admin?.contactNumber || "Not provided"} />
                </div>
                <div className="flex items-start gap-3">
                  <Mail className="h-4 w-4 mt-1 text-muted-foreground" />
                  <InfoRow label="Email" value={admin?.email || "Not provided"} />
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-muted/50 p-4 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <InfoRow label="Created On" value={formatDateTime(admin?.createdAt || "")} />
                </div>
                <div className="flex items-start gap-3">
                  <Calendar className="h-4 w-4 mt-1 text-muted-foreground" />
                  <InfoRow label="Last Updated" value={formatDateTime(admin?.updatedAt || "")} />
                </div>
                <div className="flex items-start gap-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <InfoRow label="Account Status" value={admin?.isDeleted ? "Inactive" : "Active"} />
                </div>
              </div>
            </div>
          </div>
