...
const AdminFormDialog = ({open, onClose, onSuccess, admin}: IAdminFormDialogProps) => {
  ...
  const [state, formAction, isPending] = useActionState(isEdit ? updateAdmin.bind(null, admin?.id as string) : createAdmin, null);
        ...
        <form ref={formRef} action={formAction} className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto px-6 space-y-4 pb-4">
            ...
            <Field>
              ...
              <Input id="name" name="name" placeholder="John Doe" defaultValue={state?.formData?.name || admin?.name || ""} />
            ...
            <Field>
              ...
              <Input
                ...
                defaultValue={state?.formData?.contactNumber || admin?.contactNumber || ""}
              />
            ...
            {!isEdit && (
              <Field>
                ...
                <Input id="password" name="password" type="password" placeholder="Enter password" defaultValue={state?.formData?.password || ""} />
            ...
            {!isEdit && (
              <Field>
                ...
                {selectedFile && (
                  <div className="mb-2">
                    <Image src={URL.createObjectURL(selectedFile)} alt="Profile Photo Preview" width={50} height={50} className="rounded-full" />
                  </div>
                )}

                <Input ref={fileInputRef} id="file" name="file" type="file" accept="image/*" onChange={handleFileChange} />
                <p className="text-xs text-gray-500 mt-1">Upload a profile photo for the admin</p>
