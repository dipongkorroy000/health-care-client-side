...
const AdminTable = ({admins}: AdminsTableProps) => {
  ...
  const handleView = (admin: IAdmin) => setViewingAdmin(admin);

  const handleEdit = (admin: IAdmin) => setEditingAdmin(admin);

  const handleDelete = (admin: IAdmin) => setDeletingAdmin(admin);
  ...
  return (
    <>
      ...
      <AdminViewDetailDialog open={!!viewingAdmin} onClose={() => setViewingAdmin(null)} admin={viewingAdmin} />
