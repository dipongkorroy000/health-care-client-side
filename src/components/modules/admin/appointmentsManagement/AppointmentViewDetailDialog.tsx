...
const AppointmentViewDetailDialog = ({open, onClose, appointment}: AppointmentViewDetailDialogProps) => {
  if (!appointment) return null;

  const getStatusBadge = () => {
    if (appointment.status === AppointmentStatus.SCHEDULED) return <Badge variant="default">Scheduled</Badge>;

    if (appointment.status === AppointmentStatus.INPROGRESS) return <Badge variant="secondary">In Progress</Badge>;
  ...
  const startDate = appointment.schedule ? new Date(appointment.schedule.startDateTime) : null;
  const endDate = appointment.schedule ? new Date(appointment.schedule.endDateTime) : null;
        ...
        <ScrollArea className="max-h-[70vh]">
          <div className="space-y-6 pr-4">
            ...
            <div>
              ...
              <div className="space-y-2 ml-6">
                <div>
                  <div className="text-sm font-medium text-muted-foreground">Video Calling ID</div>
                  <div className="font-mono">{appointment.videoCallingId || "Not set"}</div>
                </div>
                <div className="flex gap-4">
                  <div className="flex-1">
                    <div className="text-sm font-medium text-muted-foreground flex items-center gap-1">
                    ...
                    <div>{appointment.paymentStatus === PaymentStatus.PAID ? "Paid" : "Unpaid"}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <div>{appointment.createdAt ? new Date(appointment.createdAt).toLocaleString() : "N/A"}</div>
              </div>
              <div>
                <div className="text-muted-foreground">Updated At</div>
                <div>{appointment.updatedAt ? new Date(appointment.updatedAt).toLocaleString() : "N/A"}</div>
              </div>
            </div>
          </div>
        </ScrollArea>
