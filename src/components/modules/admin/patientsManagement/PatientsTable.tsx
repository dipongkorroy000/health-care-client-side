...
const PatientsTable = ({patients}: PatientsTableProps) => {
  ...
  const handleView = (patient: IPatient) => setViewingPatient(patient);

  const handleEdit = (patient: IPatient) => setEditingPatient(patient);

  const handleDelete = (patient: IPatient) => setDeletingPatient(patient);
  ...
  const confirmDelete = async () => {
    ...
    if (result.success) {
      ...
      handleRefresh();
    } else toast.error(result.message || "Failed to delete patient");
  };
  ...
  return (
    <>
      ...
      <PatientViewDetailDialog open={!!viewingPatient} onClose={() => setViewingPatient(null)} patient={viewingPatient} />
