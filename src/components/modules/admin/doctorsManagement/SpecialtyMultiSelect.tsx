        ...
        <div className="flex flex-wrap gap-2 mb-3 p-3 bg-muted rounded-lg">
          {selectedSpecialtyIds?.map((id) => (
            <Badge key={id} variant="secondary" className="px-3 py-1.5 text-sm">
              ...
              <Button variant="link" onClick={() => onRemoveSpecialty(id)} className="ml-2 hover:text-destructive">
                <X className="h-3 w-3" />
              </Button>
            </Badge>
          ))}
          ...
          <SelectContent>
            ...
            {availableSpecialties?.length <= 0 && (
              <SelectItem value="none" disabled>
                {selectedSpecialtyIds?.length > 0 ? "All specialties selected" : "No specialties available"}
              </SelectItem>
            )}
          </SelectContent>
        </Select>

        <Button type="button" onClick={onAddSpecialty} disabled={!currentSpecialtyId} variant="outline">
          Add
        </Button>
      </div>

      <p className="text-xs text-gray-500 mt-1">{isEdit ? "Add new specialties or remove existing ones" : "Select at least one specialty for the doctor"}</p>

      {/* Edit Mode: Show Changes */}
      {isEdit && (
        <div className="mt-2 space-y-1">
          {getNewSpecialties()?.length > 0 && <p className="text-xs text-green-600">✓ Will add: {getNewSpecialties()?.map(getSpecialtyTitle)?.join(", ")}</p>}
          {removedSpecialtyIds?.length > 0 && <p className="text-xs text-red-600">✗ Will remove: {removedSpecialtyIds?.map(getSpecialtyTitle)?.join(", ")}</p>}
        </div>
      )}
    </Field>
  );
