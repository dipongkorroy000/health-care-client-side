...
const DoctorFilters = ({specialties}: DoctorsFilterProps) => {
  ...
  useEffect(() => {
    ...
    if (debouncedContactNumber) params.set("contactNumber", debouncedContactNumber);
  ...
  const toggleSpecialty = (specialtyId: string) => {
    const newSelection = localSpecialties.includes(specialtyId) ? localSpecialties.filter((id) => id !== specialtyId) : [...localSpecialties, specialtyId];
  ...
  const applySpecialtyFilter = () => {
    ...
    if (localSpecialties.length > 0) localSpecialties.forEach((val) => params.append("specialties", val));
      ...
      <div className="flex items-center gap-3">
        ...
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <Button variant="outline" role="combobox" aria-expanded={open} className="w-60 justify-between h-10">
              <Filter className="mr-2 h-4 w-4" />
              {localSpecialties.length > 0 ? `${localSpecialties.length} selected` : "Select specialties"}
          ...
          <PopoverContent className="w-60 p-0" align="start">
            <Command>
              ...
              <CommandList>
                ...
                <CommandGroup>
                  {specialties.map((specialty) => {
                    const isSelected = localSpecialties.includes(specialty.title);
                    return (
                      <CommandItem
                        key={specialty.id}
                      ...
                      >
                        ...
                        <span className={isSelected ? "font-medium" : ""}>{specialty.title}</span>
              ...
              <div className="p-2 border-t">
                <Button onClick={applySpecialtyFilter} className="w-full" size="sm" disabled={isPending}>
                  Apply Filter
                </Button>
              </div>
            </Command>
        ...
        {/* Gender Filter */}
        <Select value={genderInput} onValueChange={(value) => setGenderInput(value === "all" ? "" : value)} disabled={isPending}>
