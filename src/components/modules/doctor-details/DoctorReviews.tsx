...
import {getReviews} from "@/services/patient/review.service";
...
export default function DoctorReviews({doctorId}: {doctorId: string}) {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          if (response.data.length > 0) {
            const total = response.data.reduce((sum: number, review: Review) => sum + review.rating, 0);
            setStats({
  ...
  return (
    <Card>
        ...
        <div className="flex items-center justify-between">
          ...
          {stats.totalReviews > 0 && (
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1">{renderStars(Math.round(stats.averageRating))}</div>
      ...
      <CardContent>
        {reviews.length === 0 ? (
          <p className="text-muted-foreground text-center py-8">No reviews yet. Be the first to review this doctor!</p>
        ) : (
              ...
              <div key={review.id} className="border-b last:border-0 pb-4 last:pb-0">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium">{review.patient?.name || "Anonymous"}</p>
