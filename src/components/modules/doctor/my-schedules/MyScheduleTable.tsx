...
export default function MySchedulesTable({schedules = []}: MySchedulesTableProps) {
  ...
  const [deletingSchedule, setDeletingSchedule] = useState<IDoctorSchedule | null>(null);
  ...
  const handleDelete = (schedule: IDoctorSchedule) => {
    ...
    if (!schedule.isBooked) setDeletingSchedule(schedule);
    else toast.error("Cannot delete booked schedule");
  };
  ...
  const confirmDelete = async () => {
    ...
    if (result.success) {
      ...
      handleRefresh();
    } else toast.error(result.message || "Failed to delete schedule");
  };
