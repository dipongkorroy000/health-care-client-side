...
export default function BookScheduleDialog({open, onClose, onSuccess, availableSchedules: initialAvailableSchedules = []}: BookScheduleDialogProps) {
  ...
  useEffect(() => {
    if (open) loadAvailableSchedules();
  ...
  const handleToggleSchedule = (scheduleId: string) => {
    setSelectedSchedules((prev) => (prev.includes(scheduleId) ? prev.filter((id) => id !== scheduleId) : [...prev, scheduleId]));
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Select time slots from available schedules to add to your calendar</DialogDescription>
        </DialogHeader>
          ...
          {loadingSchedules ? (
            ...
            <div className="space-y-6">
              {groupedSchedules.map(([date, daySchedules]) => (
                <div key={date}>
                  <h3 className="font-medium mb-3">{format(new Date(date), "EEEE, MMMM d, yyyy")}</h3>
        ...
        <DialogFooter>
          <div className="flex items-center justify-between w-full">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={handleSubmit} disabled={selectedSchedules.length === 0 || isLoading}>
