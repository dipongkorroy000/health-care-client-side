...
export default function ChangeAppointmentStatusDialog({appointment, isOpen, onClose}: ChangeStatusDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (newStatus === AppointmentStatus.COMPLETED && !appointment.prescription) {
          setTimeout(() => toast.info("Don't forget to provide a prescription for this patient", {duration: 5000}), 1000);
        }
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>Update the status for {appointment.patient?.name}&apos;s appointment</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <div className="text-sm font-medium">{statusOptions.find((opt) => opt.value === appointment.status)?.label}</div>
          </div>

          {/* New Status */}
          <div className="space-y-2">
            <Label htmlFor="status">New Status</Label>
            <Select value={newStatus} onValueChange={(value) => setNewStatus(value as AppointmentStatus)} disabled={isSubmitting}>
          ...
          {newStatus === AppointmentStatus.COMPLETED && !appointment.prescription && (
