...
export default function DoctorAppointmentsTable({appointments = []}: DoctorAppointmentsTableProps) {
  ...
  const handleView = (appointment: IAppointment) => setViewingAppointment(appointment);

  const handleStatusChange = (appointment: IAppointment) => setChangingStatusAppointment(appointment);
  ...
  const handleEditClick = (appointment: IAppointment) => {
    ...
    if (appointment.status === AppointmentStatus.CANCELED) {
      toast.error("Cannot change status for canceled appointments", {description: "Canceled appointments are final and cannot be modified."});
      return;
    }

    if (appointment.status === AppointmentStatus.COMPLETED && !!appointment.prescription) {
      toast.error("Cannot change status once prescription is provided", {
        description: "Appointment status is locked after prescription is created to maintain medical record integrity.",
      });
      return;
    }
