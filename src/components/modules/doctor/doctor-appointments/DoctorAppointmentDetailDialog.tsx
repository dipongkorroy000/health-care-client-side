...
import {createPrescription} from "@/services/patient/prescriptions.service";
...
export default function DoctorAppointmentDetailDialog({appointment, open, onClose}: DoctorAppointmentDetailDialogProps) {
  ...
  const {patient, schedule, status, paymentStatus, prescription} = appointment;
  ...
  const handleSubmitPrescription = async () => {
    ...
    if (instructions.trim().length < 20) {
      toast.error("Instructions must be at least 20 characters long for clarity");
      return;
    }
    ...
    try {
      const prescriptionData: {appointmentId: string; instructions: string; followUpDate?: string} = {
        appointmentId: appointment.id,
        instructions: instructions.trim(),
      };

      // Convert to ISO-8601 DateTime format
      if (followUpDate) prescriptionData.followUpDate = new Date(followUpDate).toISOString();
      ...
      if (result.success) {
        ...
        onClose();
        router.refresh();
      } else toast.error(result.message || "Failed to create prescription");
    } catch (error) {
          ...
          <div className="border rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-medium">{schedule?.startDateTime ? format(new Date(schedule.startDateTime), "PPP") : "N/A"}</p>
              </div>
              <div>
              ...
              <div className="space-y-4">
                <div className="bg-amber-50 border border-amber-200 rounded-md p-3 mb-4">
                  <p className="text-sm text-amber-800">
                    ⚠️ Once created, prescriptions cannot be edited or deleted. Please ensure all information is correct.
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">{instructions.length} / 20 characters minimum</p>
                </div>
                ...
                <Button onClick={handleSubmitPrescription} disabled={isSubmitting} className="w-full">
            ...
            {hasPrescription && (
              <div className="space-y-4">
                <div className="bg-green-50 border border-green-200 rounded-md p-3 mb-4">
                  <p className="text-sm text-green-800">✓ Prescription has been provided for this appointment</p>
                  <p className="text-xs text-green-700 mt-1">Note: Appointment status cannot be changed once prescription is provided</p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  {prescription.followUpDate && (
                    <div>
                      ...
                      <p className="text-sm font-medium">{format(new Date(prescription.followUpDate), "PPP")}</p>
                    </div>
                  )}

                  <div className="text-xs text-muted-foreground italic pt-2 border-t">Note: Prescriptions cannot be edited or deleted once created</div>
                </div>
              </div>
            )}
          </div>
