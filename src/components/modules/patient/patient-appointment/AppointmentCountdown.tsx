...
const AppointmentCountdown = ({appointmentDateTime, className = ""}: AppointmentCountdownProps) => {
  ...
  return (
    <div className={`flex items-center gap-2 text-sm ${isPast ? "text-red-600" : "text-blue-600"} ${className}`}>
      <Clock className="h-4 w-4" />
      <span className="font-medium">{isPast ? "Appointment passed" : `Starts in: ${timeLeft}`}</span>
    </div>
  );
};
