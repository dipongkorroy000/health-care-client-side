...
import {Calendar, Clock, CreditCard, FileText, Loader2, MapPin, MessageSquare, Star, Stethoscope, User} from "lucide-react";
      ...
      <Card className="border-dashed">
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-muted-foreground text-center max-w-sm">
            You haven&apos;t booked any appointments. Browse our doctors and book your first consultation.
          </p>
          ...
          <CardContent className="pt-6 space-y-4">
            ...
            <div>
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-lg">{appointment.doctor?.name || "N/A"}</h3>
                  <p className="text-sm text-muted-foreground">{appointment.doctor?.designation || "Doctor"}</p>
                </div>
              </div>
            </div>
            ...
            {appointment.doctor?.doctorSpecialties && appointment.doctor.doctorSpecialties.length > 0 && (
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {appointment.doctor.doctorSpecialties.slice(0, 2).map((ds, idx) => (
            ...
            {appointment.schedule && (
              <div className="space-y-2 bg-gray-50 rounded-lg p-3">
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="font-medium">{format(new Date(appointment.schedule.startDateTime), "EEEE, MMM d, yyyy")}</span>
                </div>
                <div className="flex items-center gap-2 text-sm">
                ...
                {appointment.status === AppointmentStatus.SCHEDULED && appointment.schedule.startDateTime && (
                  <div className="pt-2 border-t border-gray-200">
                    <AppointmentCountdown appointmentDateTime={appointment.schedule.startDateTime} />
                  </div>
                )}
              </div>
            )}
            ...
            {appointment.doctor?.address && (
              <div className="flex items-start gap-2 text-sm text-muted-foreground">
                ...
                <span className="line-clamp-2">{appointment.doctor.address}</span>
              </div>
            )}
            ...
            {appointment.status === AppointmentStatus.COMPLETED && (
              <div>
                {appointment.review ? (
                  ...
                  <div className="text-sm text-muted-foreground bg-gray-50 rounded-lg p-2">No review yet</div>
                )}
              </div>
            )}
          </CardContent>
          ...
          <CardFooter className="border-t pt-4">
            <div className="flex gap-2 w-full">
              <Button variant="outline" size="sm" className="flex-1" asChild>
                <Link href={`/dashboard/my-appointments/${appointment.id}`}>View Details</Link>
              </Button>
              {appointment.paymentStatus === PaymentStatus.UNPAID && appointment.status !== AppointmentStatus.CANCELED && (
                <Button
