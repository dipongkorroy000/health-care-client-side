  ...
  return (
    <div className={`flex flex-col items-center gap-4 ${className} justify-center min-h-screen`}>
      ...
      <div className={`relative ${container} ${!animated ? "no-animation" : ""}`}>
        {/* Outer Pulsing Ring */}
        {animated && <div className="pulse-ring absolute inset-0 rounded-full border-[3px] border-blue-500" />}
        ...
        <div className="absolute inset-[15%] flex items-center justify-center">
          {showIcon ? (
            <div className={animated ? "heartbeat-icon" : ""}>
              <Activity className="text-blue-500" size={iconSize} strokeWidth={2} />
            </div>
          ) : (
            <svg className="w-full h-full" style={{transform: "scaleX(-1)"}} fill="none" preserveAspectRatio="xMidYMid meet" viewBox="0 0 58 58">
              <path
        ...
        {animated && <div className="pulse-glow absolute inset-[15%] rounded-full bg-blue-500/20" />}
      </div>

      {/* Text */}
      {text && <p className={`font-medium text-gray-600 text-center ${textSize} ${animated ? "fade-in-text" : ""}`}>{text}</p>}
    </div>
  );
}
