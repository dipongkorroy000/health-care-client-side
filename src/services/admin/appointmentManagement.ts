...
export async function changeAppointmentStatus(id: string, status: string) {
  try {
    const response = await server_fetch.patch(`/appointment/status/${id}`, {
    ...
    return {success: false, message: process.env.NODE_ENV === "development" ? error.message : "Failed to change status"};
  }
}
