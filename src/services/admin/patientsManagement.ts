...
export async function getPatients(queryString?: string) {
  try {
    const response = await server_fetch.get(`/patient${queryString ? `?${queryString}` : ""}`);
...
export async function getPatientById(id: string) {
  try {
    const response = await server_fetch.get(`/patient/${id}`);
    const result = await response.json();
...
export async function updatePatient(id: string, _prevState: any, formData: FormData) {
  ...
  try {
    const response = await server_fetch.patch(`/patient/${id}`, {
...
export async function softDeletePatient(id: string) {
  try {
    const response = await server_fetch.delete(`/patient/soft/${id}`);
    const result = await response.json();
...
export async function deletePatient(id: string) {
  try {
    const response = await server_fetch.delete(`/patient/${id}`);
    const result = await response.json();
