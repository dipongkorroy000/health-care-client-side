...
export async function updateMyProfile(formData: FormData) {
  try {
    ...
    const response = await server_fetch.patch(`/user/update-my-profile`, {body: uploadFormData});
...
export async function getNewAccessToken() {
  try {
    ...
    } else throw new Error("No Set-Cookie header found");

    if (!accessTokenObject || !refreshTokenObject) throw new Error("Tokens not found in cookies");
