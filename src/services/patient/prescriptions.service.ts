...
export async function createPrescription(data: IPrescriptionFormData) {
  try {
    ...
    return {success: false, message: process.env.NODE_ENV === "development" ? error.message : "Failed to create prescription"};
  }
}
...
export async function getMyPrescriptions(queryString?: string) {
  try {
    ...
    return {success: false, data: [], message: process.env.NODE_ENV === "development" ? error.message : "Failed to fetch prescriptions"};
  }
}
